...
export function LoseCard({
  ...
  return (
    <div
      ref={cardRef}
      className="w-full max-w-2xl mx-auto bg-[#111111] border border-[#2a2a2a] border-t-4 border-t-gray-600 p-6 sm:p-12 text-center flex flex-col items-center shadow-2xl rounded-sm"
    >
      <div className="mb-8 w-full flex flex-col items-center">
        <div className="text-gray-400 text-xs sm:text-sm uppercase tracking-widest font-bold mt-2">
          {siteContent.loseCard.subtitle}
        </div>
        <div className="text-2xl sm:text-3xl font-black tracking-widest text-white drop-shadow-md mt-6 mb-2">
          {siteContent.loseCard.comeBackText}
          <span className="text-tp-red">
            {siteContent.loseCard.comeBackHighlight}
          </span>
        </div>
        <div className="text-gray-500 text-xs uppercase tracking-widest font-bold">
          {siteContent.loseCard.findOut}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 w-full mb-10 text-sm sm:text-base font-medium text-gray-300">
        <div className="bg-[#1a1a1a] py-4 px-6 border border-[#222] rounded-sm flex items-center justify-center shadow-inner w-full">
          <p className="text-center leading-relaxed uppercase tracking-wider text-xs font-bold text-gray-500">
            ...
            {siteContent.loseCard.stats.playersGuessed}
          </p>
        </div>
        ...
        <div className="bg-[#1a1a1a] py-4 px-6 border border-[#222] rounded-sm flex items-center justify-center shadow-inner w-full">
          <p className="text-center leading-relaxed uppercase tracking-wider text-xs font-bold text-gray-500">
            ...
            {siteContent.loseCard.stats.surrendersLabel}
          </p>
        </div>

        <div className="bg-[#1a1a1a] py-4 px-6 border border-[#222] rounded-sm flex items-center justify-center shadow-inner w-full">
          <p className="text-center leading-relaxed uppercase tracking-wider text-xs font-bold text-gray-500">
            {siteContent.loseCard.stats.gaveUp1}
            <span className="text-tp-red font-black text-lg">{userTries}</span>
            {siteContent.loseCard.stats.gaveUp2}
          </p>
        </div>
      </div>
      ...
      {stats && stats.tries_distribution && (
        <div className="w-full mb-10 flex flex-col items-center bg-[#1a1a1a] p-6 sm:p-8 border border-[#222] rounded-sm shadow-inner">
          ...
          <div className="w-full max-w-md opacity-40 grayscale pointer-events-none">
            <ChartStat distribution={stats.tries_distribution} userTries={-1} />
          </div>
        </div>
      )}
